...
@include('backend.partials.content-header', ['router' => 'setting.index', 'name' => 'Setting', 'key' => 'Edit'])
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('setting.update', $setting->id)}}" method="post">
            ...
            <div class="form-group">
                <label>Config value</label>
                <textarea class="form-control @error('config_value') is-invalid @enderror" name="config_value" id="configvalue" placeholder="Nhập config value" rows="5">{{$setting->config_value}}</textarea>
                @error('config_value')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
